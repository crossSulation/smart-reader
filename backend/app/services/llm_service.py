...
def _mock_complete(prompt: str, system: Optional[str]) -> CompletionResult:
    ...
    if "knowledge extraction engine" in system_text or "knowledge graph linker" in system_text:
        raise RuntimeError(
            "Knowledge extraction requires a real LLM. "
            "Set LLM_PROVIDER=openai (or ollama) and configure LLM_API_KEY / LLM_MODEL in .env.dev."
        )
